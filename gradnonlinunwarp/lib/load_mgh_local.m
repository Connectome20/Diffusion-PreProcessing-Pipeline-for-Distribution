...
function [vol, M, M0_vox2tkr, mr_parms, delta] = load_mgh_local(fname,slices,frames,headeronly)
% [vol, M0_vox2ras, M0_vox2tkr, mr_parms, delta] = load_mgh(fname,<slices>,<frames>,<headeronly>)
%
...
%
% M0 is the 4x4 vox2ras transform such that
...
% edited version of "load_mgh.m" rev 1.20, Original Author: Bruce Fischl
  ...
  if (strcmpi(fname((strlen(fname)-3):strlen(fname)), '.MGZ') | ...
    ...
    if(strcmp(computer,'MAC') || strcmp(computer,'MACI') || ismac)
      [status,result] = unix(sprintf('gunzip -c %s > %s', fname, new_fname)) ;
    else
      [status,result] = unix(sprintf('zcat %s > %s', fname, new_fname)) ;
    end
    fname = new_fname ;
  else
    gzipped = -1 ;
  ...
  if(isempty(v))
    ...
    if(gzipped >=0),  [status,result] = unix(sprintf('rm %s', fname)); end
  end
  ...
  if (ras_good_flag)
    ...
    Mdc    = reshape(Mdc,[3 3]);
    Pxyz_c = fread(fid, 3, 'float32');  % cras
    ...
    % LOCAL VERSION:
    M0_vox2tkr = [[-delta(1), 0, 0; 0, 0, +delta(3); 0 -delta(2) 0], [+delta(1)*Pcrs_c(1);-delta(3)*Pcrs_c(3);+delta(2)*Pcrs_c(2)]; [0 0 0 1]];
  ...
  %************************************************************************%
  %%% $Source: /space/repo/1/dev/dev/gradient_nonlin_unwarp/load_mgh_local.m,v $
  %%% Local Variables:
  %%% mode: Matlab
  %%% fill-column: 76
  %%% comment-column: 0
