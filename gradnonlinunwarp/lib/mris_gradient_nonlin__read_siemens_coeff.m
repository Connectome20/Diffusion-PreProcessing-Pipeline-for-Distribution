...
%MRIS_GRADIENT_NONLIN__READ_SIEMENS_COEFF  spherical harmonics from coeff_X.grad
%
...
%
%   [Alpha_x, Alpha_y, Alpha_z, Beta_x, Beta_y, Beta_z, R0_m] = mris_gradient_nonlin__read_siemens_coeff('coeff_X.grad');
...
%**************************************************************************%
  ...
  disp(sprintf('==> [%s]: reading system type string from coeff_X.grad file...', mfilename));
  disp(systemtype_str);
  ...
  %************************************************************************%
  %%% $Source: /space/repo/1/dev/dev/gradient_nonlin_unwarp/mris_gradient_nonlin__read_siemens_coeff.m,v $
  %%% Local Variables:
  %%% mode: Matlab
  %%% fill-column: 76
  %%% comment-column: 0
