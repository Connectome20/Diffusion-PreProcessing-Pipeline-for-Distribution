...
% $Id: mris_gradient_nonlin__unwarp.m,v 1.3 2013/07/21 17:16:32 jonp Exp $
%**************************************************************************%
  ...
  switch method,
   case 'direct',
    ...
    if ( d == 0 ),
      ...
      VJacDet = mris_gradient_nonlin__jacobian_determinant(DVx, DVy, DVz, [], VX, VY, VZ);
    end;
   ...
   case 'lookup',
    ...
    [LDx, LDy, LDz, LJacDet, L1xyz2rcs] = mris_gradient_nonlin__load_gradtable(gradtablefile);

    % image volume voxels projected into gradient lookup-table indices
    [LVR, LVC, LVS] = mris_transform_coordinates(VX, VY, VZ, L1xyz2rcs);


    %==-------------

    LVRmin = min(LVR(:)); LVRmax = max(LVR(:));
    LVCmin = min(LVC(:)); LVCmax = max(LVC(:));
    LVSmin = min(LVS(:)); LVSmax = max(LVS(:));

    if ( (LVRmin < 1) || (LVRmax > size(LDx,1)) || ...
	 (LVCmin < 1) || (LVCmax > size(LDy,1)) || ...
	 (LVSmin < 1) || (LVSmax > size(LDz,1)) ),
      warning('input volume dimensions exceed bounds of gradient table');
    end;


    %==-------------

    % values of gradient lookup table evaluated at locations of image voxels
    ...
    tic;
    DVx     = interp3(LDx, LVC, LVR, LVS, 'cubic', 0);
    DVy     = interp3(LDy, LVC, LVR, LVS, 'cubic', 0);
    DVz     = interp3(LDz, LVC, LVR, LVS, 'cubic', 0);

    % not sure why the interpolated, resampled Jacobian approach does not work...
    %VJacDet = interp3(LJacDet, LVC, LVR, LVS, 'cubic');
    VJacDet = mris_gradient_nonlin__jacobian_determinant(DVx, DVy, DVz, [], VX, VY, VZ);
