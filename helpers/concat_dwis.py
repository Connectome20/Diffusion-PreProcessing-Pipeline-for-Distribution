...
def concatenate_nifti_with_mrcat(file_paths, output_file):
    ...
    if missing_files:
        raise FileNotFoundError(f"Missing input files for concatenation: {missing_files}")
...
def concatenate_dwi_data(subj_dir, subject_id, output_base):
    ...
    for i, path in enumerate(nii_paths):
        if os.path.exists(path):
            ...
            existing_order.append(order[i])
        else:
            print(f"Warning: File not found: {path}")
    
    if not existing_paths:
        raise FileNotFoundError(f"No DWI files found for subject {subject_id} in {subj_dir}")
    
    # Update order and paths to only include existing files
    order = existing_order
    nii_paths = existing_paths
    # n_volumes_per_file = [nib.load(f).header.get_data_shape()[-1] for f in nii_paths]
    n_volumes_per_file = [nib.load(f).shape[3] if len(nib.load(f).shape) == 4 else 1 for f in nii_paths]
