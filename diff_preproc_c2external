...
def update_paths(*args):
    ...
    # Get the bids_path from bids_path_entry
    bids_path = bids_path_entry.get("1.0", 'end-1c').strip()
    ...
    if subject_ids and bids_path:
        # For single subject, use specific path. For multiple subjects, use template
        if len(subject_ids) == 1:
            subject_id = subject_ids[0]
            # Update paths for the single subject
            new_dwi_path = f"{bids_path}/sub-{subject_id}/dwi"
            new_dwi_process_path = f"{bids_path}/sub-{subject_id}/dwi_process"
            new_anat_process_path = f"{bids_path}/sub-{subject_id}/anat_process"
            new_fsInput_path = f"{bids_path}/sub-{subject_id}/anat/sub-{subject_id}_run-02_T1w.nii.gz"
            new_image_to_run_gnc_path = f"{bids_path}/sub-{subject_id}/anat/sub-{subject_id}_run-01_T1w.nii.gz"
        else:
            # Use template for multiple subjects
            new_dwi_path = f"{bids_path}/{{subj_id}}/dwi"
            new_dwi_process_path = f"{bids_path}/{{subj_id}}/dwi_process"
            new_anat_process_path = f"{bids_path}/{{subj_id}}/anat_process"
            new_fsInput_path = f"{bids_path}/{{subj_id}}/anat/{{subj_id}}_run-02_T1w.nii.gz"
            new_image_to_run_gnc_path = f"{bids_path}/{{subj_id}}/anat/{{subj_id}}_run-01_T1w.nii.gz"
...
def execute_dcm2bids():
    ...
    for subject_id in subject_ids:
        ...
        cmd = dcm2bids_command(subject_id, bids_path, config, dcm_source)
...
def mppca_noise():
    subject_ids = get_subject_ids()
    sequence_type = gnc_option.get()
    selected_flow_value = selected_flow.get()
    
    if sequence_type == "A":
        gnc_cmd = "/your/project/directory/bids/code/preprocessing_dwi/your_gnc_script_A.sh"
    elif sequence_type == "B":
        gnc_cmd = "/your/project/directory/bids/code/preprocessing_dwi/your_gnc_script_B.sh"
    else:
        status_text.insert('end', f"Unknown sequence type: {sequence_type}. Skipping GNC processing.\n")
        return
    ...
    for subject_id in subject_ids:
        ...
        gnc_sigma_cmd = f"{gnc_cmd} -i {concat_dir}/sub-{subject_id}_sigma.nii.gz -o {noise_dir}/sub-{subject_id}_sigma.nii.gz -interp spline"
        ...
        gnc_mask_cmd = f"{gnc_cmd} -i {mask_dir}/brain_mask.nii.gz -o {mask_dir}/sub-{subject_id}_brain_mask_gnc.nii.gz -interp nn"
        gnc_wm_cmd = f"{gnc_cmd} -i {mask_dir}/wm.nii.gz -o {mask_dir}/sub-{subject_id}_wm_gnc.nii.gz -interp nn"

        for cmd in [dwiextract_cmd, dwidenoise_cmd, gnc_sigma_cmd, gnc_mask_cmd, gnc_wm_cmd]:
...
def append_all_commands_to_script():
    ...
    # 1. DCM2BIDS
    dcm2bids_cmd = dcm2bids_command(subject_id, bids_path_entry.get("1.0", 'end-1c').strip(), config_entry.get("1.0", 'end-1c').strip(), dcm_source_entry.get("1.0", 'end-1c').strip())
    ...
    # 11. recon-all
    expertFile = "/your/project/directory/bids/code/preprocessing_dwi/expertFile"
    ...
    # Set sequence-specific GNC command
    sequence_type = gnc_option.get()
    if sequence_type == "A":
        gnc_cmd = "/your/project/directory/bids/code/preprocessing_dwi/your_gnc_script_A.sh"
    else:
    ...
    gnc_sigma_cmd = f"{gnc_cmd} -i {concat_dir}/sub-{subject_id}_sigma.nii.gz -o {base_dir}/{noise_dir_suffix}/sub-{subject_id}_sigma.nii.gz -interp spline"
    ...
    for cmd in [dwiextract_cmd, dwidenoise_cmd, gnc_A_cmd, gnc_mask_cmd,gnc_wm_cmd]:
        append_command_to_script(cmd, base_dir, subject)
...
#    """
#    Reads a file and returns a list of values from the first line.
...
def find_indices(phase_encoding_file, diffusion_time_file, bval_file, sequence_type):
    """
    Finds the indices based on phase encoding, diffusion time, and b-values.
    Handles sequence A and B data with different criteria for diffusion times.
    """
    ...
    # Diffusion time criteria based on sequence type
    if sequence_type == "A":
        dt_D = "33" # Replace with the D specific to your sequence
        dt_d = "22" # Replace with the d specific to your sequence
    elif sequence_type == "B":
        dt_D = "11" # Replace with the D specific to your sequence
        dt_d = "00" # Replace with the d specific to your sequence
    else:
        raise ValueError("Invalid sequence type. Must be 'A' or 'B'.")

    # Indices for different categories
    indices_b0s_PA = [i for i, pe in enumerate(phase_encodings) if pe == "-1"]
    indices_DXms = [i for i, (pe, dt) in enumerate(zip(phase_encodings, diffusion_times)) if pe == "1" and dt == dt_D]
    indices_DYms = [i for i, (pe, dt) in enumerate(zip(phase_encodings, diffusion_times)) if pe == "1" and dt == dt_d]
    
    indices_dx_b2400_less = [i for i, (pe, dt, bv) in enumerate(zip(phase_encodings, diffusion_times, bvals)) if pe == "1" and dt == dt_D and 0 < float(bv) < 2400]
    indices_dx_b2400_greater = [i for i, (pe, dt, bv) in enumerate(zip(phase_encodings, diffusion_times, bvals)) if pe == "1" and dt == dt_D and float(bv) >= 2400]
    indices_dy_b2400_less = [i for i, (pe, dt, bv) in enumerate(zip(phase_encodings, diffusion_times, bvals)) if pe == "1" and dt == dt_d and 0 < float(bv) < 2400]
    indices_dy_b2400_greater = [i for i, (pe, dt, bv) in enumerate(zip(phase_encodings, diffusion_times, bvals)) if pe == "1" and dt == dt_d and float(bv) >= 2400]

    # Debug: Print out the length of indices
    print(f"Indices for b0s PA ({sequence_type}):", len(indices_b0s_PA))
    print(f"Indices for DT={dt_D}ms ({sequence_type}):", len(indices_DXms))
    print(f"Indices for DT={dt_d}ms ({sequence_type}):", len(indices_DYms))
    print(f"Indices for DT={dt_D}, BV<2400 ({sequence_type}):", len(indices_dx_b2400_less))
    print(f"Indices for DT={dt_D}, BV>=2400 ({sequence_type}):", len(indices_dx_b2400_greater))
    print(f"Indices for DT={dt_d}, BV<2400 ({sequence_type}):", len(indices_dy_b2400_less))
    print(f"Indices for DT={dt_d}, BV>=2400 ({sequence_type}):", len(indices_dy_b2400_greater))

    return (
        indices_b0s_PA, 
        indices_DXms, 
        indices_DYms,
        indices_dx_b2400_less, 
        indices_dx_b2400_greater, 
        indices_dy_b2400_less, 
        indices_dy_b2400_greater
    )

def get_sequence_type():
    """
    Determines the sequence type (A or B) based on the BIDS path provided in the GUI.
    """
    base_dir = bids_path_entry.get("1.0", "end-1c").strip()
    if "seqA" in base_dir:
        return "A"
    elif "seqB" in base_dir:
        return "B"
    else:
        raise ValueError("Unable to determine sequence type from base path. Ensure it contains 'seqA' or 'seqB'.")
...
def run_mrconvert(input_file, bvec_file, bval_file, indices, output_file, output_bvec_file, output_bval_file, subject_id, folder_name):
    ...
    """
    base_dir = bids_path_entry.get("1.0", "end-1c").strip()  # Dynamically get the base path from the C2 entry
...
def execute_sandi_volume_division():
    """
    Executes the DWI volume processing for SANDI based on sequence type.
    """
    subject_ids = get_subject_ids()
    ...
    for subject_id in subject_ids:
        ...
        try:
            indices_b0s_PA, indices_DXms, indices_DYms, _, _, _, _ = find_indices(
                phase_encoding_file, diffusion_time_file, bval_file, sequence_type
            )
            ...
            run_mrconvert(input_file, bvec_file, bval_file, indices_DXms, "DXms.nii.gz", "bvecs_DXms.txt", "bvals_DXms.txt", subj_id, "s13_SANDI")
            run_mrconvert(input_file, bvec_file, bval_file, indices_DYms, "DYms.nii.gz", "bvecs_DYms.txt", "bvals_DYms.txt", subj_id, "s13_SANDI")
...
def execute_tractcaliber_volume_division():
    """
    Executes the DWI volume processing for TractCaliber based on sequence type.
    """
    subject_ids = get_subject_ids()
    sequence_type = get_sequence_type()
    ...
    for subject_id in subject_ids:
        ...
        try:
            indices_b0s_PA, _, _, indices_dx_b2400_less, indices_dx_b2400_greater, indices_dy_b2400_less, indices_dy_b2400_greater = find_indices(
                phase_encoding_file, diffusion_time_file, bval_file, sequence_type
            )
            ...
            run_mrconvert(input_file, bvec_file, bval_file, range(min(indices_dx_b2400_less)-1,max(indices_dx_b2400_less)+1), "qb32_DXms.nii.gz", "bvecs_qb32_DXms.txt", "bvals_qb32_DXms.txt", subj_id, "s10_TractCaliber")
            run_mrconvert(input_file, bvec_file, bval_file, range(min(indices_dx_b2400_greater)-1,max(indices_dx_b2400_greater)+1), "qb64_DXms.nii.gz", "bvecs_qb64_DXms.txt", "bvals_qb64_DXms.txt", subj_id, "s10_TractCaliber")
            run_mrconvert(input_file, bvec_file, bval_file, range(min(indices_dy_b2400_less)-1,max(indices_dy_b2400_less)+1), "qb32_DYms.nii.gz", "bvecs_qb32_DYms.txt", "bvals_qb32_DYms.txt", subj_id, "s10_TractCaliber")
            run_mrconvert(input_file, bvec_file, bval_file, range(min(indices_dy_b2400_greater)-1,max(indices_dy_b2400_greater)+1), "qb64_DYms.nii.gz", "bvecs_qb64_DYms.txt", "bvals_qb64_DYms.txt", subj_id, "s10_TractCaliber")
...
def run_tractseg_for_subject(subject_id):
    ...
    dwi_nii = f"{dwi_dir}/DYms.nii.gz"
    bval_txt = f"{dwi_dir}/bvals_DYms.txt"
    bvec_txt = f"{dwi_dir}/bvecs_DYms.txt"
...
# BIDS Path Entry
tk.Label(main_frame, text="BIDS Path:").grid(row=1, column=0, sticky='w')
bids_path_entry, bids_path_entry_frame = make_horizontal_scrollable_entry(main_frame, width=50, height=1)
bids_path_entry_frame.grid(row=1, column=1, sticky='ew')
bids_path_entry.insert('1.0', "/your/project/directory/bids")
# Bind the key release event to the bids_path_entry widget
bids_path_entry.bind("<KeyRelease>", lambda event: update_paths())
...
def create_file_entry_widgets(parent):
    ...
    for i, label in enumerate(dwi_file_labels, start=10):  # Adjust start index based on your layout
        ...
        def open_file_dialog(entry=entry):
            ...
            subject_id = subj_entry.get("1.0", 'end-1c').strip()
            initial_directory = f"/autofs/cluster/msilab/users/oakley//sub-{subject_id}/dwi"
...
concatenate_dwi_button = ttk.Button(main_frame, text="Concat DWI data from Entries", command=concatenate_dwi_data_from_gui_entries,style='Large.TButton',width=30)
...
tk.Label(main_frame, text="Select Option:").grid(row=30, column=0, sticky='w')

# Variable to store the selected GNC option
gnc_option = tk.StringVar(value="A")
...
gnc_c1_rb = tk.Radiobutton(gnc_option_radio_button_frame, text="Type A", variable=gnc_option, value="A", background='lightblue')
gnc_c1_rb.pack(side='left', padx=5)

gnc_c2_rb = tk.Radiobutton(gnc_option_radio_button_frame, text="Type B", variable=gnc_option, value="B", background='lightblue')
...
tk.Label(main_frame, text="Select Option:").grid(row=37, column=0, sticky='w')
gnc_anat_option = tk.StringVar(value="A")  # Default value is "A"
...
gnc_c1_anat_rb = tk.Radiobutton(gnc_anat_radio_button_frame, text="Type A", variable=gnc_anat_option, value="A", background='lightblue')
gnc_c1_anat_rb.pack(side='left', padx=5)

gnc_c2_anat_rb = tk.Radiobutton(gnc_anat_radio_button_frame, text="Type B", variable=gnc_anat_option, value="B", background='lightblue')
...
execute_dwi_volume_processing_button.grid(row=47, column=1, padx=(10, 0))  # Adjust the row and column as needed
ttk.Separator(main_frame, orient='horizontal').grid(row=48, columnspan=5, sticky="ew")
...
execute_tractcaliber_dwi_volume_processing_button.grid(row=49, column=1, padx=(10, 0))  # Adjust the row and column as needed
ttk.Separator(main_frame, orient='horizontal').grid(row=50, columnspan=5, sticky="ew")

# Add the "Run TractSeg" Button to the GUI
# Button to execute TractSeg for all subjects
run_tractseg_button = ttk.Button(
    main_frame, 
    text="Run TractSeg for All Subjects", 
    command=run_tractseg_for_all_subjects,  # Set the new function as the command
    style='Large.TButton', 
    width=30
)
run_tractseg_button.grid(row=51, column=1, padx=(10, 0))  # Adjust row and column based on your layout
ttk.Separator(main_frame, orient='horizontal').grid(row=52, columnspan=5, sticky="ew")
...
buttons_frame.grid(row=53, column=0, columnspan=2, pady=(10, 0), sticky='ew')
...
# Create a separator
ttk.Separator(main_frame, orient='horizontal').grid(row=54, columnspan=5, sticky="ew", pady=(10, 5))
...
steps_frame.grid(row=55, column=0, columnspan=2, pady=(5, 10), padx=10, sticky='ew')

# Create checkbox variables for each step
step_dcm2bids = tk.BooleanVar(value=False)
step_export_params = tk.BooleanVar(value=False)
step_concatenate = tk.BooleanVar(value=False)
step_denoise = tk.BooleanVar(value=False)
step_degibbs = tk.BooleanVar(value=False)
step_topup = tk.BooleanVar(value=False)
step_masks = tk.BooleanVar(value=False)
step_eddy = tk.BooleanVar(value=False)
step_gnc = tk.BooleanVar(value=False)
step_interpolation = tk.BooleanVar(value=False)
step_mppca_noise = tk.BooleanVar(value=False)
step_write_output = tk.BooleanVar(value=False)
step_gnc_anat = tk.BooleanVar(value=False)
step_recon_all = tk.BooleanVar(value=False)
...
checkboxes = [
    ...
    ("Exp Diff Params", step_export_params),
    ("Concat DWI", step_concatenate),
    ("MPPCA Denoise", step_denoise),
    ("DeGibbs", step_degibbs),
    ("TopUp Corr", step_topup),
    ("WM/Brain Masks", step_masks),
    ("Eddy Corr", step_eddy),
    ("GNC DWI", step_gnc),
    ("Interpolation", step_interpolation),
    ("MPPCA Noise", step_mppca_noise),
    ("Write Output", step_write_output),
    ("GNC Anat", step_gnc_anat),
    ("recon-all", step_recon_all),
    ("SANDI Vol Div", step_sandi_division),
    ("TC Vol Div", step_tractcaliber),
...
tk.Label(main_frame, text="PA Volume Index:").grid(row=60, column=0, sticky='w')
pa_volume_entry, pa_volume_entry_frame = make_horizontal_scrollable_entry(main_frame, width=5, height=1)
pa_volume_entry_frame.grid(row=60, column=1, sticky='w')
pa_volume_entry.insert('1.0', "9")  # default to 9

tk.Label(main_frame, text="AP Volume Index:").grid(row=61, column=0, sticky='w')
ap_volume_entry, ap_volume_entry_frame = make_horizontal_scrollable_entry(main_frame, width=5, height=1)
ap_volume_entry_frame.grid(row=61, column=1, sticky='w')
